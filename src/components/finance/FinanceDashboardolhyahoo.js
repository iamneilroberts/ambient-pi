...
const FinanceDashboard = () => {
  ...
  // Market indices hardcoded for initial state
  const [indices, setIndices] = useState([
    { name: 'S&P 500', symbol: '^GSPC', value: '0.00', change: '0.00' },
    { name: 'NASDAQ', symbol: '^IXIC', value: '0.00', change: '0.00' },
    { name: 'DOW', symbol: '^DJI', value: '0.00', change: '0.00' }
  ]);
  ...
  const fetchData = async (showLoader = true) => {
    try {
      ...
      const indexResults = await Promise.allSettled(
        indices.map(index => financeApi.fetchStockData(index.symbol))
      );
      
      const updatedIndices = indices.map((index, i) => {
        const result = indexResults[i];
        if (result.status === 'fulfilled') {
          return {
            ...index,
            value: result.value.currentPrice.toFixed(2),
            change: result.value.percentChange.toFixed(2)
          };
        }
        return index;
      });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1">
          <div className="bg-gray-800 rounded-lg p-4 h-full">
            <MarketNews symbols={stocks} baseUrl="http://localhost:3005" />
          </div>
        </div>
      </div>
    </div>
